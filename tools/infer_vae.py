...
def infer(args):
    ...
    ngrid = int(math.sqrt(num_images))
    ...
    with torch.no_grad():
        ...
        resize_transform = torchvision.transforms.Resize((512, 512), interpolation=torchvision.transforms.InterpolationMode.NEAREST)

        # Apply the transform to the PIL Image
        encoder_grid = resize_transform(encoder_grid)
        decoder_grid = resize_transform(decoder_grid)
        input_grid = resize_transform(input_grid)
